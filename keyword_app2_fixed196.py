...
# 적용된 값(Apply 버튼 기준)
    ...
    if kwdno_list and str(kwdno_list).strip():
        # 쉼표로 나눈 각 값에 대해, 앞뒤 공백과 괄호()를 제거한 뒤 비교용 코드로 사용
        raw_parts = [x for x in str(kwdno_list).split(",") if str(x).strip()]
        _targets = []
        for _part in raw_parts:
            t = str(_part).strip().upper()
            # 앞뒤 괄호/공백 제거: (US087), US087), (US087 등도 모두 US087로 정규화
            t = re.sub(r"^[\s\(\)]+", "", t)
            t = re.sub(r"[\s\(\)]+$", "", t)
            if t:
                _targets.append(t)
